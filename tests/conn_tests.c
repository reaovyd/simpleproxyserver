...
Test(conn_suite, conn_get_fd_set_1) {
    ConnectionPool *conn_pool = conn_pool_init();
    CONNPOOL_NOTNULL(conn_pool);
    cr_assert_eq(conn_copy_fd_sets(NULL, NULL, NULL), -1, "Expected copy_fd to fail, but got a succeeding status"); 
}
...
Test(conn_suite, conn_get_fd_set_2) {
    ...
    int ccfs_status = conn_copy_fd_sets(conn_pool, &rd_cpy, &wr_cpy);
